...
style dlc_btn_text:
    ...
    outlines []
transform dlc_slide_anim(delay=0.0):
...
init 2 python:
    def dlc_slide(delay=0.0):
        # На «низких» графике анимации интерфейса отключаем.
        try:
            if dlc_quality() == "low":
                return Transform()
        except Exception:
            pass
        return dlc_slide_anim(delay)
...
screen dlc_select_screen():
    ...
    add "dlc_menu_bg"
    $ menu_snow = dlc_menu_snow()
    if menu_snow is not None:
        add menu_snow
    if _dlc_logo:
        add _dlc_logo:
            xpos 104
        ...
        frame:
            ...
            vbox:
                ...
                hbox:
                    spacing 16
                    xalign 0.5
                    for q_opt, q_label in (("low", u"НИЗКИЕ"), ("medium", u"СРЕДНИЕ"), ("high", u"ВЫСОКИЕ")):
                        textbutton q_label:
                            xysize (372, 72)
                            text_size 34
                            text_kerning 2.0
                            action Function(dlc_set_quality, q_opt)
                            selected (dlc_quality() == q_opt)
                            background dlc_frame(372, 72, "#7d94ab7a", 2, "#050a1059")
                            hover_background dlc_frame(372, 72, "#ffffff", 2, "#0c1a2799")
                            selected_background dlc_frame(372, 72, "#c8d8e8", 2, "#12283cdd")
                            selected_hover_background dlc_frame(372, 72, "#ffffff", 2, "#12283cdd")
                            text_idle_color "#c2cfdc"
                            text_hover_color "#ffffff"
                            text_selected_color "#ffffff"
                text dlc_quality_hint():
                    size 28
                    color "#8c9bab"
                    xalign 0.5
                    text_align 0.5
